...
import { buildInitialState, cardMatches, createDeck, GameEngine } from './game.js';
const teams = ['brazil', 'argentina', 'france', 'portugal'];
const assets = teams.flatMap((teamId) => [
...
const c = (type, extra = {}) => ({ id: id(), type, ...extra });
function stateWith(players, drawPile = [c('number', { number: 9, teamId: 'brazil' })]) {
    return {
...
describe('effects', () => {
    it('Warning draws 2 and increments Warning; second Warning converts to Red while still drawing 2', () => {
        ...
        expect(s.players[1].warnings).toBe(1);
        expect(s.players[1].hand).toHaveLength(2);
        expect(s.currentPlayerIndex).toBe(1);
        s.players[1].warnings = 1;
        s.players[0].hand.push(c('warning', { teamId: 'brazil' }));
        ...
        expect(s.players[1].warnings).toBe(0);
        expect(s.players[1].reds).toBe(1);
    ...
    it('Switch exchanges exactly one card and the Switch owner decides', () => {
        const own = c('number', { number: 4, teamId: 'brazil' });
        const theirs = c('number', { number: 7, teamId: 'argentina' });
        const sw = c('switch', { teamId: 'brazil' });
        const s = stateWith([{ hand: [sw, own] }, { hand: [theirs, c('number', { number: 8, teamId: 'argentina' })] }]);
        const e = new GameEngine(s, opts());
        e.dispatch({ type: 'playCard', playerId: 'p1', cardId: sw.id });
        e.dispatch({ type: 'switchCards', playerId: 'p1', ownCardId: own.id, targetCardId: theirs.id });
        expect(s.players[0].hand.map(x => x.id)).toContain(theirs.id);
        expect(s.players[1].hand.map(x => x.id)).toContain(own.id);
        expect(s.players[0].hand).toHaveLength(2);
        expect(s.players[1].hand).toHaveLength(2);
    });
    it('Switch can be passed without an exchange', () => {
        const sw = c('switch', { teamId: 'brazil' });
        const s = stateWith([{ hand: [sw] }, {}]);
        const e = new GameEngine(s, opts());
        e.dispatch({ type: 'playCard', playerId: 'p1', cardId: sw.id });
        e.dispatch({ type: 'passSwitch', playerId: 'p1' });
        expect(s.currentPlayerIndex).toBe(1);
        expect(s.players[0].hand).toHaveLength(0);
    });
    it('Red draws 4, is wild, and third accumulated Red resets counter and skips the target next turn', () => {
        const red = c('red');
        const s = stateWith([{ hand: [red] }, { reds: 2 }], [c('number', { number: 1, teamId: 'brazil' }), c('number', { number: 2, teamId: 'brazil' }), c('number', { number: 3, teamId: 'brazil' }), c('number', { number: 4, teamId: 'brazil' })]);
        const e = new GameEngine(s, opts());
        e.dispatch({ type: 'playCard', playerId: 'p1', cardId: red.id });
        expect(s.players[1].reds).toBe(0);
        expect(s.players[1].hand).toHaveLength(4);
        expect(s.skipNext).toBe(true);
        expect(s.currentPlayerIndex).toBe(0);
        e.dispatch({ type: 'chooseTeam', playerId: 'p1', teamId: 'argentina' });
        expect(s.skipNext).toBe(false);
        expect(s.currentPlayerIndex).toBe(0);
        expect(s.currentTeamId).toBe('argentina');
    });
});
describe('penalty and goalkeeper', () => {
    it('no goalkeeper: attacker chooses Team and no extra draw occurs', () => {
        const penalty = c('penalty');
        const s = stateWith([{ hand: [penalty] }, { hand: [] }]);
        const e = new GameEngine(s, opts());
        e.dispatch({ type: 'playCard', playerId: 'p1', cardId: penalty.id });
        e.dispatch({ type: 'chooseTeam', playerId: 'p1', teamId: 'france' });
        expect(s.currentTeamId).toBe('france');
        expect(s.players[1].hand).toHaveLength(0);
        expect(s.currentPlayerIndex).toBe(1);
    });
    it('goalkeeper Save: accept, hidden attacker choice, correct guess succeeds', () => {
        const penalty = c('penalty');
        const gk = c('number', { number: 1, teamId: 'argentina', playerId: 'argentina-1' });
        const s = stateWith([{ hand: [penalty] }, { goalkeeperCard: gk }]);
        let now = 10_000;
        const e = new GameEngine(s, { ...opts(), now: () => now });
        e.dispatch({ type: 'playCard', playerId: 'p1', cardId: penalty.id });
        ...
        expect(s.penalty?.chosenTeamId).toBe('portugal');
        expect(s.penalty?.deadlineAt).toBe(15_000);
        e.dispatch({ type: 'guessTeam', playerId: 'p2', teamId: 'portugal' });
        expect(s.penalty).toBeUndefined();
        expect(s.currentTeamId).toBe('portugal');
    });
    it('wrong guess draws 1 and applies attacker Team', () => {
        ...
        e.dispatch({ type: 'attemptSave', playerId: 'p2', accept: true });
        e.dispatch({ type: 'chooseTeam', playerId: 'p1', teamId: 'portugal' });
        e.dispatch({ type: 'guessTeam', playerId: 'p2', teamId: 'france' });
    ...
    it('timeout is a failed defense', () => {
        ...
        e.dispatch({ type: 'chooseTeam', playerId: 'p1', teamId: 'portugal' });
        now = 7000;
        expect(e.resolvePenaltyTimeout()).toBe(true);
        expect(s.players[1].hand).toHaveLength(1);
        expect(s.currentTeamId).toBe('portugal');
    });
});
describe('last card and ranking', () => {
    it('applies last-card function effect before finishing', () => {
        const warning = c('warning', { teamId: 'brazil' });
        const s = stateWith([{ hand: [warning] }, {}], [c('number', { number: 1, teamId: 'brazil' }), c('number', { number: 2, teamId: 'brazil' })]);
    ...
    it('ranks by cards, warnings, reds, then number sum and preserves full ties', () => {
        ...
        const e = new GameEngine(s, opts());
        const r = e.calculateRanking();
        expect(r.map(x => [x.rank, x.playerId])).toEqual([[1, 'p1'], [2, 'p2'], [3, 'p3'], [4, 'p4']]);
        const tie = stateWith([{ hand: [] }, { hand: [] }]);
        expect(new GameEngine(tie, opts()).calculateRanking().map(x => x.rank)).toEqual([1, 1]);
    });
});
describe('reshuffle', () => {
    it('reuses discard except top card without losing the top', () => {
        const draw = c('number', { number: 2, teamId: 'brazil' });
        const old = c('number', { number: 3, teamId: 'brazil' });
        const top = c('number', { number: 4, teamId: 'brazil' });
        const s = stateWith([{ hand: [] }, {}], [draw]);
        s.discardPile = [old, top];
        const e = new GameEngine(s, opts());
        e.dispatch({ type: 'drawCard', playerId: 'p1' });
        expect(s.discardPile).toHaveLength(1);
        expect(s.discardPile[0].id).toBe(top.id);
